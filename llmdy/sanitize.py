...
def clean_html(html: str, clean_svg=True, clean_base64=True):
    """
    ...
    """
    return str(BeautifulSoup(sanitizer.clean_html(html, clean_svg, clean_base64))).strip()
    ...
    def extract_html(content: str) -> str:
        ...
        for header in MD_HEADERS:
            if content.startswith(header):
                return content[len(header) + 1: -len(MD_CODE_TRAIL)].strip()
        return content
    if incomplete_md:
